...
describe('parse', () => {
  it.each([
    ...
    [
      'Command with implicit boolean flag value',
      'algolia profile add --default',
      {
        error: undefined,
        parsedCommand: {
          command: ['algolia', 'profile', 'add'],
    ...
    [
      'Command with implicit boolean flag value after argument',
      'algolia indices config export ARG1 ARG2 ARG3 --confirm',
      {
        error: undefined,
        parsedCommand: {
          command: ['algolia', 'indices', 'config', 'export'],
